...
def build_ai_summary(report_date, weakest_zone_name, weakest_zone_score, most_missing_ppe_name, priority_task_text):
    ...
    items = []
    if w_name != "-": items.append(f"{report_date} 기준 가장 취약한 구역은 {w_name}이며 해당 구역 내 위반비율은 {weakest_zone_score}%입니다.")
    if p_name != "-": items.append(f"반복 누락 PPE는 {p_name}입니다. 해당 보호구 착용 확인을 우선 강화해야 합니다.")
    if t_text != "-": items.append(f"우선 개입 필요 작업은 {t_text}입니다.")
...
.section-card { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 22px; padding: 20px; box-shadow: 0 8px 24px rgba(15, 23, 42, 0.05); margin-bottom: 1rem; }
.section-title { font-size: 1.16rem; font-weight: 800; color: #0f172a; margin-bottom: 0.25rem; }
.section-sub { color: #64748b; font-size: 0.88rem; margin-bottom: 0.8rem; }
.metric-card { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 20px; padding: 18px 18px; min-height: 158px; box-shadow: 0 8px 24px rgba(15, 23, 42, 0.05); }
...
.info-box { border-radius: 15px; padding: 14px 15px; border: 1px solid #fecaca; background: #fff1f2; color: #b91c1c; line-height: 1.65; font-size: 0.93rem; font-weight: 600; margin-bottom: 0.7rem; }
.sub-box { border-top: 1px solid #e2e8f0; margin-top: 1rem; padding-top: 1rem; }
.sub-box-title { font-weight: 800; color: #475569; margin-bottom: 0.55rem; font-size: 0.98rem; }
.ai-title-row { display: flex; align-items: center; gap: 10px; margin-bottom: 0.25rem; }
.ai-emoji { font-size: 1.25rem; line-height: 1; }
.ai-title { font-size: 1.08rem; font-weight: 800; color: #0f172a; }
.ai-sub { color: #64748b; font-size: 0.88rem; margin-bottom: 0.8rem; }
.ai-list { margin: 0; padding-left: 1rem; }
.ai-list li { margin-bottom: 0.6rem; color: #334155; line-height: 1.7; font-size: 0.94rem; word-break: keep-all; }
.ai-updated { color: #6b7280; font-size: 0.84rem; font-weight: 600; margin-top: 0.75rem; }
.caption-note { color: #64748b; font-size: 0.88rem; margin-top: 0.3rem; }
...
st.markdown('<div class="main-subtitle">날짜와 현장 조건에 맞는 안전 현황을 확인하세요</div>', unsafe_allow_html=True)
...
if not hourly_df.empty:
    ...
    hourly_df["time_slot"] = pd.Categorical(hourly_df["time_slot"], categories=["오전", "점심직후", "오후"], ordered=True)
    hourly_df = hourly_df.sort_values("time_slot")

# 💡 구역별 데이터 스마트 가공 (초록색 준수율 + 빨간색 위험도 만들기)
...
if not zone_data.empty:
    ...
    zone_data["compliance"] = (100.0 - zone_data["risk"]).round(2) # 100에서 빼서 준수율 만듦!
    zone_data = zone_data.sort_values("risk", ascending=False).reset_index(drop=True)
else:
    zone_data = pd.DataFrame(columns=["zone", "risk", "compliance"])
...
with row1_col1:
    st.markdown('<div class="section-card"><div class="section-title">시간대별 PPE 이탈 건수</div><div class="section-sub">시간대별 API 집계 결과입니다. 눈금 간격 2건 고정</div>', unsafe_allow_html=True)
    fig_time = go.Figure()
    if not hourly_df.empty:
        fig_time.add_trace(go.Bar(x=hourly_df["time_slot"].astype(str).tolist(), y=hourly_df["count"].tolist(), orientation="v", marker=dict(color=["#3b82f6", "#60a5fa", "#1d4ed8"]), hovertemplate="시간대: %{x}<br>건수: %{y}건<extra></extra>"))
    y_max = max(4, (int(hourly_df["count"].max()) if not hourly_df.empty else 0) + 2)
    fig_time.update_layout(height=330, margin=dict(l=10, r=10, t=10, b=10), plot_bgcolor="white", paper_bgcolor="white", showlegend=False, bargap=0.45, xaxis=dict(showgrid=False, tickfont=dict(size=12, color="#334155"), categoryorder="array", categoryarray=["오전", "점심직후", "오후"]), yaxis=dict(showgrid=True, gridcolor="#e2e8f0", zeroline=False, tickfont=dict(size=11, color="#475569"), range=[0, y_max], dtick=2))
    try: fig_time.update_layout(barcornerradius=30)
    except Exception: pass
    st.plotly_chart(fig_time, use_container_width=True, config={"displayModeBar": False})
    st.markdown('</div>', unsafe_allow_html=True)

with row1_col2:
    st.markdown('<div class="section-card"><div class="section-title">오늘의 안전개입 포인트</div>', unsafe_allow_html=True)
    for point in safety_points[:3]: st.markdown(f'<div class="info-box">⚠️ 안전 포인트<br><span style="font-weight:500;">{point}</span></div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-box"><div class="sub-box-title">권장 조치사항</div><ul><li>점심 직후 현장 순찰 강화</li><li>고소작업 전 랜야드 체결 확인</li><li>절단작업 전 장갑 착용 점검</li></ul></div></div>', unsafe_allow_html=True)

# =========================
# 2행 (작업구역별 위험도 - 완벽한 % 나란히 차트!)
# =========================
row2_col1, row2_col2 = st.columns([1.85, 1.15])
with row2_col1:
    st.markdown('<div class="section-card"><div class="section-title">구역별 위험노출 준수율</div><div class="section-sub">구역별 준수율과 위험도를 100% 기준으로 비교합니다</div>', unsafe_allow_html=True)
    fig_zone = go.Figure()
    if not zone_data.empty:
        # 💡 초록색(준수율) 추가
        fig_zone.add_trace(go.Bar(y=zone_data["zone"].tolist(), x=zone_data["compliance"].tolist(), name="준수율 %", orientation="h", marker=dict(color="#22c55e"), hovertemplate="구역: %{y}<br>준수율: %{x}%<extra></extra>"))
        # 💡 빨간색(위험도) 추가
        fig_zone.add_trace(go.Bar(y=zone_data["zone"].tolist(), x=zone_data["risk"].tolist(), name="위험도 %", orientation="h", marker=dict(color="#ef4444"), hovertemplate="구역: %{y}<br>위험도: %{x}%<extra></extra>"))
    ...
    fig_zone.update_layout(
        ...
        xaxis=dict(range=[0, 100], dtick=20, gridcolor="#e2e8f0", zeroline=False, tickfont=dict(size=11, color="#475569"), ticksuffix="%"),
        yaxis=dict(autorange="reversed", showgrid=False, tickfont=dict(size=12, color="#334155"))
    )
    
    try: fig_zone.update_layout(barcornerradius=10)
    except Exception: pass
    
    st.plotly_chart(fig_zone, use_container_width=True, config={"displayModeBar": False})
    st.markdown(f'<div class="caption-note">{weakest_zone_name} 구역 내 위반비율 {weakest_zone_score}% · 0~100% 기준 고정</div></div>', unsafe_allow_html=True)

with row2_col2:
    st.markdown('<div class="section-card"><div class="ai-title-row"><div class="ai-emoji">🧠</div><div class="ai-title">AI 안전 분석</div></div><div class="ai-sub">실시간 패턴 분석 기반 요약</div><ul class="ai-list">', unsafe_allow_html=True)
