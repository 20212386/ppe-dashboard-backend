...
def calc_axis_max(series: pd.Series, base: int = 5, minimum: int = 10):
    if series.empty:
        return minimum
    max_value = int(series.max())
    return max(minimum, ((max_value + base - 1) // base + 1) * base)
...
with r1c1:
    ...
    if analysis_data is None:
        ...
        y_max_time = calc_axis_max(time_df["count"] if not time_df.empty else pd.Series(dtype=int), base=5, minimum=10)
        ...
        try:
            fig_time.update_layout(barcornerradius=10)
        except Exception:
            pass

        st.plotly_chart(fig_time, use_container_width=True, config={"displayModeBar": False}, key="p3_time_chart")
        st.markdown(
            f'<div class="insight-box" style="background:#eff6ff; border-color:#bfdbfe; color:#1e3a8a;">💡 패턴 해석: 가장 위반이 집중된 시간대는 <b>{top_time}</b>입니다.</div>',
            unsafe_allow_html=True
        )
...
with r1c2:
    ...
    if analysis_data is None:
        ...
        if not ppe_df.empty:
            ...
            ppe_colors = [ppe_color_map.get(label, "#94a3b8") for label in ppe_df["label"]]
        ...
        y_max_ppe = calc_axis_max(ppe_df["count"] if not ppe_df.empty else pd.Series(dtype=int), base=5, minimum=10)
        ...
        try:
            fig_ppe.update_layout(barcornerradius=10)
        except Exception:
            pass

        st.plotly_chart(fig_ppe, use_container_width=True, config={"displayModeBar": False}, key="p3_ppe_chart")
        st.markdown(
            f'<div class="insight-box" style="background:#fff7ed; border-color:#fed7aa; color:#9a3412;">💡 패턴 해석: 가장 많이 누락되는 보호구는 <b>{top_ppe}</b>입니다.</div>',
            unsafe_allow_html=True
        )
...
with r2c1:
    ...
    if analysis_data is None:
        ...
        if not zone_df.empty:
            ...
            fig_zone.add_trace(
                go.Bar(
                    ...
                    marker=dict(
                        color=zone_colors,
                        line=dict(color="#2563eb", width=0.4)
                    ),
                    width=0.44,
        ...
        x_max_zone = calc_axis_max(zone_df["count"] if not zone_df.empty else pd.Series(dtype=int), base=5, minimum=10)
        ...
        try:
            fig_zone.update_layout(barcornerradius=10)
        except Exception:
            pass

        st.plotly_chart(fig_zone, use_container_width=True, config={"displayModeBar": False}, key="p3_zone_chart")
        st.markdown(
            f'<div class="insight-box" style="background:#eff6ff; border-color:#bfdbfe; color:#1e3a8a;">💡 패턴 해석: 가장 취약한 구역은 <b>{top_zone}</b>입니다.</div>',
            unsafe_allow_html=True
        )
...
with r2c2:
    ...
    if analysis_data is None:
        ...
        if not task_df.empty:
            fig_task.add_trace(
                go.Bar(
                    ...
                    marker=dict(
                        color="#8b5cf6",
                        line=dict(color="#7c3aed", width=0.6)
                    ),
                    width=0.44,
                    hovertemplate="%{y}: %{x}건<extra></extra>"
                )
        ...
        x_max_task = calc_axis_max(task_df["count"] if not task_df.empty else pd.Series(dtype=int), base=5, minimum=10)
        ...
        try:
            fig_task.update_layout(barcornerradius=10)
        except Exception:
            pass

        st.plotly_chart(fig_task, use_container_width=True, config={"displayModeBar": False}, key="p3_task_chart")
        st.markdown(
            f'<div class="insight-box" style="background:#faf5ff; border-color:#e9d5ff; color:#6b21a8;">💡 패턴 해석: 반복 개입 우선 작업은 <b>{top_task}</b>입니다.</div>',
            unsafe_allow_html=True
        )
...
if analysis_data is None:
    st.markdown(
        '<div class="recommend-box">필터를 설정한 뒤 <b>분석 실행</b> 버튼을 누르면 현재 조건에 맞는 개입 조치가 생성됩니다.</div>',
        unsafe_allow_html=True
    )
else:
